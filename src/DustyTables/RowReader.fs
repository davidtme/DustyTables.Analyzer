...
type RowReader(reader: SqlDataReader) =
    ...
    member this.string(column: string) : string =
        match columnDict.TryGetValue(column) with
        | true, columnIndex ->
            if types.[columnIndex] = "uniqueidentifier"
            then string (reader.GetGuid(columnIndex))
            else reader.GetString(columnIndex)
    ...
    member this.boolOrNone(column: string) : bool option =
        ...
        | true, columnIndex ->
            if reader.IsDBNull(columnIndex)
            then None
            else Some (reader.GetBoolean(columnIndex))
        | false, _ -> failToRead column "bool"
